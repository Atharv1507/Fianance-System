...
export default function EditTransactionModal({ isOpen, onClose, transaction, onUpdated }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/user/transaction/${transaction.id}`,
        {
          userId: user?.id,
      ...
      setFeedback({ type: 'error', message: err.response?.data?.message || 'Failed to update transaction.' })
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-modal-in">
        ...
        <div className="bg-gradient-to-r from-amber-500 to-orange-600 px-6 py-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-amber-100 text-sm mt-0.5">Modify your record</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                min="0.01"
                step="0.01"
                className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={handleChange}
              className="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition bg-white"
            >
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition resize-none"
            />
          </div>
        </div>
        ...
        <div className="border-t border-gray-100 px-6 py-4 bg-gray-50/50 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={saving}
            className="px-5 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white text-sm font-semibold rounded-xl hover:from-amber-600 hover:to-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-sm flex items-center gap-2"
          >
            {saving ? (
              <>
